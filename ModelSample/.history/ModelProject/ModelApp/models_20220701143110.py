...
class BaseMeta(models.Model):
    ...
    #- model更新時にwarningが出ることがあり、下記でtimezone関連データ登録時のwarningを消せる - なぜかエラーが出るようになったので使わないことにする
    create_at = models.DateTimeField(default=timezone.datetime.now(pytz.timezone('Asia/Tokyo')))
    update_at = models.DateTimeField(default=timezone.datetimenow(pytz.timezone('Asia/Tokyo')))
