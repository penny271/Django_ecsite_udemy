...
next(gen_02) # x=None yieldは返り値がないため xには何も入らないから
gen_02.send(100) # -send(): yieldで停止している箇所に値を送ります。
# gen_02.throw(ValueError('Invalid Value')) #- throw(): 指定した例外が発生して処理が終了させます。
gen_02.close() #- close(): ジェネレータを正常終了させます。
next(gen_02)