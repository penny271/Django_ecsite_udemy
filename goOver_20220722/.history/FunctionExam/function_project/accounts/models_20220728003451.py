...
# #- テーブルのデータ挿入、取り出しをするクラスは、models.Managerを継承して作成する。 user.is_active=Trueにする処理
class UserActivateTokensManager(models.Manager):

    def activate_user_by_token(self,token):
        #- レコードをtokenで一意に絞り、 user.is_active=Trueにする
...
#- views.py の def regist(request): でUsersモデルのレコードを保存する挙動が検出されたときに@receiverが発動する
@receiver(post_save, sender=Users)
def publish_token(sender, instance, **kwargs):
    print(str(uuid4()))
    print(datetime.now() + timedelta(days=1))
    user_activate_token = UserActivateTokens.objects.create(
        #- 作成したレコード(sender=Users)、この場合Usersのインスタンスが第二引数のinstanceに入る
