...
def delete_theme(request, id):
    ...
    if delete_theme_form.is_valid(): # csrf checkを行う
        ...
        return redirect('boards:list_themes')
    return render(
        #- context={'delete_theme_form':delete_theme_form}を入れる理由は、
        #- edit_theme.html の <p><a class="navbar-brand" href="{% url 'boards:delete_theme' id=id %}">削除する</a></p>で idを引数として渡すため
        request, 'boards/delete_theme.html'
        }
    )
