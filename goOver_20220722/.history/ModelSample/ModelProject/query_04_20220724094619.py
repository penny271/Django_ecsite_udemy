...
#- 外部テーブルでexclude
# for student in Students.objects.exclude(school__name='東高校'):
#     print(student.name, student.school.name, student.school.prefecture)

#^ Model.objects.filter(外部テーブル__外部テーブル__カラム=‘○○’) # 外部テーブルの外部テーブルのカラムを指定 して絞込み
# for student in Students.objects.exclude(school__prefecture__name='東京'):
#     print(student.name, student.school.name, student.school.prefecture)
...
for student in Students.objects.order_by(name):
    print(student.name)
