...
class InputAddressView(LoginRequiredMixin, CreateView):
    ...
    #- ページを表示した場合にgetが発動する
    #- カートに商品がない状態では、住所を入力するフォームのページへいけないようにする
    #- 引数にpkを追加する - input_address.htmlにてpkを引数に設定しており、そのエラーを避けるため　<a href="{% url 'stores:input_address' pk=address.id %}">{{address}}</a>
    def get(self, request, pk=None):
    # def get(self, request, **kwargs):
        cart = get_object_or_404(Carts, user_id = request.user.id)
        if not cart.cartitems_set.all():
            raise Http404('商品が入っていません')
        #- 引数にpkを追加したため、ここの引数にもpkを引数として設定する
        return super().get(request, pk)
        # return super().get(request, **kwargs)

    #- **kwargsには<a href="{% url 'stores:input_address' pk=address.id %}">{{address}}</a>の中の引数pkが入る
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        #- formsからgetで情報を受け取るため self.request.user.id
        # UnboundLocalError: local variable 'address' referenced before assignment
        address ='' #- 事前に定義しておく必要あり or 上記error発生
        address = cache.get(f'address_user_{self.request.user.id}', address)
        #- 一意に絞り込む必要あり - そのためにget_object_or_404()にuser_idとpkの2つを設定している
        pk = self.kwargs.get('pk')
        address = get_object_or_404(Addresses,pk=pk) if pk else address
    ...
    #- NOT NULL constraint failed: address.user_id 外部キーに値を入れていないためエラーが発生
    def form_valid(self, form):
        #- formのuserに値を設定している-class AddressInputForm(forms.ModelForm):でform.userを使用する
        #- CreateViewを利用しているため save()メソッドを明記しなくてもsave()が行われる
