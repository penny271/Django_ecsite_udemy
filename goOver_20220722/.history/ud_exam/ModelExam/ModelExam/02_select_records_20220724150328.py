...
for result in student_1.testresults_set.all():
    # print(dir(result))

    print(result.student.name,result.test.name, result.test.name,result.score, )

# print(student_1.name, student_1.testresults_set.first().test.name,student_1.testresults_set.first().score, )

from django.db.models import Max, Min, Avg, Count, Sum

# test_result_list = TestResults.objects.values('score', 'student', 'test')
#- GROUP BY クラス名, テスト名
