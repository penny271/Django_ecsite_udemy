...
def delete_student(request, id):
    # student = Students.objects.get('id = id')
    delete_form = forms.StudentDeleteForm(initial={'id': id})
    if request.method == 'POST':
        #!!! requestの内容を取得
        delete_form = forms.StudentDeleteForm(request.POST or None)
        if delete_form.is_valid():
            student = Students.objects.get(id = delete_form.cleaned_data['id'])
            student.delete()

    return render(request, 'form_app/delete_student.html', context={'delete_form' : delete_form})
...
def insert_multiple_students(request):
    StudentFormSet = modelformset_factory(Students,fields=3, extra=3)
    insert_form = StudentFormSet(request.POST or None, request.FILES or )
