...
#テストスコア表示
#- test_resultはインスタンス全体であるため、特定のキーで取り出す必要あり
# for test_result in student.testresults.score_set.all(): #! NG!
for test_result in student.testresults.score_set.all():
    print(student.name, student.class_fk, test_result.test.name ,test_result.score)
